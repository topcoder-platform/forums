...
class UserMetaModel extends Gdn_Model {
     ...
     */
    public function setWatchedCategoriesCount($userID) {
        $count = $this->userWatchedCategoriesCount($userID);
        Gdn::userModel()->setField($userID, 'CountWatchedCategories', $count);
        return $count;
    }

    /**
     * Get the watched categories
     *
     * @param array $where
     * @param int $limit
     * @param int $offset
     * @return Gdn_DataSet
     */
    public function getWatchedCategories($userID, $orderby ='desc' , $limit = 50, $offset = 0) {
        $query = 'select distinct c.CategoryID as CategoryID, c.LastDiscussionCommentsDate from GDN_Category c, GDN_UserMeta um '.
        'where um.Name LIKE "Preferences.%" AND UserID = '.$userID.' and Value = 1'.
        ' and CAST(SUBSTRING_INDEX(um.Name, ".", -1) as UNSIGNED) = c.CategoryID'.
        ' order by c.LastDiscussionCommentsDate '.$orderby.' limit '.$limit.' offset '.$offset;
        $result = $this->SQL->query($query)->resultArray();
        return $result;
    }

    /**
     * Gets number of watched categories specified user has.
     *
     * @param int $userID Unique ID of user for which to tally watched categories.
     * @return int Total number of watched categories.
     */
    public function userWatchedCategoriesCount($userID) {
        $query = 'select COUNT(distinct c.CategoryID) as count from GDN_Category c, GDN_UserMeta um '.
            'where um.Name LIKE "Preferences.%" AND UserID = '.$userID.' and Value = 1'.
            ' and CAST(SUBSTRING_INDEX(um.Name, ".", -1) as UNSIGNED) = c.CategoryID';
        $result = $this->SQL->query($query)->value('count');
