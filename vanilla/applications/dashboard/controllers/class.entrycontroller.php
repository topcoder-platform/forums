...
class EntryController extends Gdn_Controller {
    ...
    protected function checkOverride($type, $target, $transientKey = null) {
        ...
        $isSSOAuthenticator = $provider instanceof \Vanilla\Authenticator\SSOAuthenticator;
        if ($url) {
            switch ($type) {
                case 'Register':
                case 'SignIn':
                    // FIX: https://github.com/topcoder-platform/forums/issues/352
                    // For SSOAuthenticator: go to '/sso' to force a sso check;
                    // Don't change target url in other cases
