...
//      PagerModule::current()->Url = '/categories/archives'.rawurlencode($Category['UrlCode']).'?page={Page}';
    ...
    public function index($categoryIdentifier = '', $page = '0') {
        ...
        if ($categoryIdentifier == '') {
            ...
            //Fix: Check global permissions and then junction permissions
    ...
    public function all($Category = '', $displayAs = '') {
        ...
        if ($Category) {
            // Missing check?
            // Check permission
            $categoryData = CategoryModel::categories($Category);
            if (empty($categoryData)) {
                throw notFoundException();
            }
            $categoryData = (object)$categoryData;
            $currentCategoryID = $categoryData->CategoryID;

            //Fix: Check global permissions and then junction permissions
            $this->categoryPermission($currentCategoryID, 'Vanilla.Discussions.View');
